...
public class EmployeeWorkingHistoryService {
    ...
    public List<EmployeeWorkingHistoryDTO> findAllEmployeeWorkingHistories(Long employeeId) {
        List<EmployeeWorkingHistory> histories =  employeeWorkingHistoryRepository.findAllByEmployeeEmployeeId(employeeId)
    ...
    public void startWorking(Long employeeId) {
        ...
        if (employeeWorkingHistoryRepository.findTodayWorkingHistory(employeeId, startOfDay, endOfDay).isPresent()) {
        ...
        employeeWorkingHistory.setEmployee(employeeRepository.findById(employeeId)
                .orElseThrow(() -> new IllegalArgumentException((("다음의 Id를 가진 employee가 없습니다: " + employeeId)))));
        // 출근 시간에 현재 시간을 세팅합니다.
    ...
    public void endWorking(Long employeeId) {
        ...
        List<EmployeeWorkingHistory> recentHistories =
                employeeWorkingHistoryRepository.findHistoriesByEmployeeId(employeeId)
        ...
        Duration workingHour = Duration.between(recentHistory.getStartDateTime(), LocalDateTime.now().withNano(0));
        // 일한 시간을 수정합니다.
    ...
    public void addWorkingHistory(EmployeeWorkingHistoryDTO employeeWorkingHistoryDTO) {
        ...
        Employee employee = employeeRepository.findById(employeeId)
                .orElseThrow(() -> new IllegalArgumentException("다음의 아이디를 가진 employee가 없습니다: " + employeeId));
       // employee를 입력합니다.
        ...
        Duration duration = Duration.between(employeeWorkingHistory.getStartDateTime(), employeeWorkingHistory.getEndDateTime());
        // 음수 시간 예외처리
    ...
    public void deleteEmployeeWorkingHistory(Long employeeWorkingHistoryId) {
        EmployeeWorkingHistory findEmployeeWorkingHistory = employeeWorkingHistoryRepository.findById(employeeWorkingHistoryId)
                .orElseThrow(() -> new IllegalArgumentException("다음의 Id를 가진 employeeWorkingHistory가 없습니다: " + employeeWorkingHistoryId));
